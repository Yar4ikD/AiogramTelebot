"""
    Модуль отвечает за работу команд ТГ-бота таких, как:
...
async def register_command(dp: Dispatcher) -> None:
    ...
    # dp.register_callback_query_handler(callback=base_stop_working, text='out', state='*')
    dp.register_message_handler(callback=base_stop_working, commands=UniversalButtons.but_out_k.text, state='*')
