...
class Command(StatesGroup):
    ...
    info_start = "Для получения списка ближайших станций укажите геолокацию."
    info_select_type_stat = "Выберите тип станции:"
    info_radius = (
        f'Укажите радиус поиска в км{emoji.emojize(":backhand_index_pointing_down:")}'
    )
    conf_radius = (
        f"Если все правильно, нажмите <b>{Buttons.but_continue.text}</b>"
    ...
    error_type_station = (
        "<b>Выберите один вариант из списка.</b>\n<b>Не нужно</b> писать сообщение!"
    ...
    async def location(cls, message: types.Message, state: FSMContext) -> None:
        ...
        if message.location:
            ...
            await message.answer(
                text=cls.info_select_type_stat, reply_markup=Buttons.stations_type()
            )

        else:
            await message.reply(
                text=cls.error_location, reply_markup=Buttons.location()
            )
    ...
    async def station(cls, message: types.Message, state: FSMContext) -> None:
        ...
        if message.text in Buttons.but_stations_type:
            ...
            await message.reply(
                text=cls.error_type_station, reply_markup=Buttons.stations_type()
            )
    ...
    async def radius(cls, message: types.Message, state: FSMContext) -> None:
        ...
        """
        if (
            message.text.isnumeric() or message.text in Buttons.buttons_distance
        ):  # является ли сообщения числом(int)
            async with state.proxy() as data:
            ...
            await cls.next()
            await message.reply(
                text=cls.conf_radius, reply_markup=Buttons.tru_continue()
            )
        ...
        if (
            ...
            async with state.proxy() as data:
                ...
                result = await request(
                    lat=lat, lng=lng, station_type=station_type, distance=distance
                )

            if result:
                History.add_command(
                    command="Список ближайших станций",
                    query=query_data,
                    response=result,
                )
                ...
                await message.answer(
                    text=cls.error_result, reply_markup=Buttons.result()
                )
    ...
    async def register_command(cls, dp: Dispatcher) -> None:
        ...
        dp.register_message_handler(
            callback=cls.start, text=Buttons.but_command_3.text, state=None
        )
        dp.register_message_handler(
            callback=cls.location,
